...
export class UsersService {
  ...
  async register(registerDto: RegisterDto, response: Response) {
    ...
    if (usersWithPhoneNumber.length > 0) {
      throw new BadRequestException(
        'Энэ утасны дугаартай хэрэглэгч аль хэдийн байна!',
      );
    }
  ...
  async Login(loginDto: LoginDto) {
    ...
    if (user && (await this.comparePassword(password, user.password))) {
      ...
      return {
        ...
        refreshToken: null,
        error: {
          message: 'Хүчингүй имэйл эсвэл нууц үг',
        },
      };
    }
  }
  ...
  async forgotPassword(forgotPasswordDto: ForgotPasswordDto) {
    ...
    if (!user) {
      throw new BadRequestException('Энэ имэйлтэй хэрэглэгч олдсонгүй!');
    }
    ...
    await this.emailService.sendMail({
      email,
      subject: 'Нууц үгээ дахин сэргээх хэрэгтэй!',
      template: './forgot-password',
      name: user.name,
      activationCode: resetPasswordUrl,
    });

    return { message: `Таны нууц үгээ мартсан хүсэлт амжилттай боллоо!` };
  }
